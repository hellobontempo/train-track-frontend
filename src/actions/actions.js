...
export function fetchPrograms() {
  return (dispatch) => {
    ...
    fetch(`${baseURL}/programs`)
      ...
      .then((programs) =>
        dispatch({ type: "FETCH_TRAINING_PROGRAMS", programs })
      );
  };
}
...
export function fetchUserPrograms() {
  return (dispatch) => {
    ...
    fetch(`${baseURL}/user_programs`)
      .then((response) => response.json())
      .then((userPrograms) =>
        dispatch({ type: "FETCH_USER_PROGRAMS", userPrograms })
      );
  };
}
...
export const addUserProgram = (newProgram) => {
  return (dispatch) => {
    fetch(`${baseURL}/user_programs`, {
      ...
      .then((resp) => resp.json())
      .then((newProgram) =>
        dispatch({ type: "ADD_USER_PROGRAM", payload: newProgram })
      )
...
export const editUserProgram = (exerciseId, customProgramId) => {
  ...
  return (dispatch) => {
    fetch(`${baseURL}/custom_programs/${customProgramId}`, {
      ...
      .then((updatedProgram) =>
        dispatch({ type: "UPDATE_USER_PROGRAM", payload: updatedProgram })
      );
  };
};
